...
final class AckFrame extends Frame
{
    ...
    public function __construct(
        ...
        if (null !== $ecnCounts && 3 !== count($ecnCounts)) {
    ...
    {
        return null !== $this->ecnCounts ? FrameType::ACK_ECN : FrameType::ACK;
    }
    ...
    public function validate(): bool
    {
        return $this->validateBasicFields()
            && $this->validateAckRanges()
            && $this->validateEcnCounts();
    }

    private function validateBasicFields(): bool
    {
        return $this->largestAcked >= 0 && $this->ackDelay >= 0;
    }

    private function validateAckRanges(): bool
    {
        foreach ($this->ackRanges as $range) {
            if (!$this->isValidAckRange($range)) {
                return false;
            }
        }
    ...
    private function isValidAckRange(mixed $range): bool
    {
        if (!is_array($range) || 2 !== count($range)) {
            return false;
        }
    ...
    private function validateEcnCounts(): bool
    {
        if (null === $this->ecnCounts) {
            return true;
        }

        if (3 !== count($this->ecnCounts)) {
            return false;
        }
